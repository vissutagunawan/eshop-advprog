...
1. Test-Driven Development (TDD) telah terbukti sangat bermanfaat dalam pengembangan kode aplikasi e-shop ini. Alur TDD membantu saya memastikan bahwa setiap fitur yang diimplementasikan bekerja dengan baik dan memenuhi kebutuhan bisnis yang telah ditentukan. Dalam konteks aplikasi ini, TDD sangat efektif karena memaksa saya untuk memikirkan persyaratan sistem dengan jelas sebelum mulai menulis kode implementasi. Misalnya, pada kelas Order dan OrderService, saya mengembangkan tes-tes seperti testCreateOrderDefaultStatus() dan testCreateOrder() terlebih dahulu, yang membantu saya menentukan bagaimana kelas-kelas ini seharusnya berperilaku.
Manfaat lain yang saya rasakan adalah kemudahan dalam refactoring. Dengan adanya test suite yang komprehensif, saya dapat melakukan perubahan pada kode dengan percaya diri, mengetahui bahwa tes akan memberitahu saya jika ada fungsionalitas yang rusak. Namun, jika ada perbaikan yang bisa dilakukan ke depannya, saya akan meningkatkan cakupan tes untuk menyertakan lebih banyak skenario edge case, seperti bagaimana aplikasi menangani input yang tidak valid atau kondisi error lainnya. Saya juga akan lebih memperhatikan integrasi antar-komponen untuk memastikan bahwa mereka bekerja dengan baik bersama-sama dalam sistem secara keseluruhan.

> 2. You have created unit tests in Tutorial. Now reflect whether your tests have successfully followed F.I.R.S.T. principle or not. If not, explain things that you need to do the next time you create more tests.

2. Setelah mengevaluasi unit test yang saya buat, saya melihat bahwa sebagian besar tes sudah mengikuti prinsip F.I.R.S.T, namun masih ada beberapa area yang perlu ditingkatkan:
Fast: Tes-tes yang ada dijalankan dengan cepat karena tidak bergantung pada operasi I/O yang lambat seperti database atau jaringan. Hal ini positif karena memungkinkan feedback loop yang cepat saat mengembangkan fitur.
Independent: Ada beberapa masalah dalam hal independensi tes. Beberapa tes seperti di OrderRepositoryTest tampaknya memiliki ketergantungan implisit pada urutan eksekusi. Metode testSaveUpdate() bergantung pada keberhasilan operasi sebelumnya. Di masa depan, saya perlu memastikan setiap tes benar-benar independen dengan mengatur ulang state yang diperlukan di dalam setiap metode tes, bukan mengandalkan state dari tes sebelumnya.
Repeatable: Tes-tes ini tampaknya repeatble, namun ada beberapa potensi masalah. Misalnya, penggunaan timestamp konkret (1708560000L) dapat menyebabkan masalah jika ada logika yang bergantung pada waktu saat ini. Ke depannya, saya harus menggunakan teknik seperti mock untuk waktu atau memastikan bahwa tes tidak bergantung pada waktu tertentu.
Self-validating: Tes-tes ini self-validating karena menggunakan assertions yang jelas untuk memverifikasi hasil yang diharapkan, seperti assertEquals dan assertThrows. Ini adalah aspek positif dari test suite.
Timely: Tes-tes ini ditulis bersamaan dengan kode implementasi, sesuai dengan alur TDD, yang merupakan praktik yang baik. Namun, saya perlu memastikan bahwa di masa depan, semua fitur baru selalu didahului oleh tes yang sesuai.
Untuk pengembangan selanjutnya, saya akan berfokus pada membuat tes yang lebih independen dengan menghindari state sharing antar tes, meningkatkan repeatability dengan menghindari ketergantungan pada faktor eksternal, dan mempertahankan pendekatan timely dengan selalu menulis tes sebelum implementasi.